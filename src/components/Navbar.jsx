...
export default function Navbar() {
  ...
  const getLinkStyle = (path) => {
    const baseStyle = "px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300";
    ...
    return `${baseStyle} ${currentPath === path ? activeStyle : inactiveStyle}`;
  };
  return (
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div className="flex justify-between items-center h-20">
      ...
      <div className="flex items-center bg-[#161616] rounded-full p-1.5 border border-[#262626]"> 
        <Link className={getLinkStyle('/')} to="/">الرئيسية</Link> 
        <Link className={getLinkStyle('/blog')} to="/blog">المدونة</Link> 
        <Link className={getLinkStyle('/about')} to="/about">من نحن</Link> 
      </div> 
    </div>
